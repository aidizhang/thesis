...
function build_siamese_model(idx_to_embedding)
	if use_batch then
		...
		twin:add(nn.Sequencer(nn.LSTM(opt.embed_size, opt.hidden_size)))
		twin:add(nn.SelectTable(1, -1)) -- selects last hidden state of LSTM, with dims (minibatch_size x hidden_size)
		print(twin.weight)
		...
		projection:add(nn.Linear(opt.hidden_size, opt.hidden_size):noBias())
		...
		-- twin:add(emb)
		twin:add(nn.SplitTable(1)) -- split tensor into seq_len tables containing vectors of size embedding_size
		twin:add(nn.Sequencer(nn.LSTM(opt.embed_size, opt.hidden_size)))
		twin:add(nn.SelectTable(-1)) -- selects last hidden state of LSTM
...
function main()
	...
	for epoch_num = 1,epochs do
		for batch_num = 0,torch.floor(1000000/batch_size)-1 do
			...
			shuffle = torch.randperm(train_contexts:size(1))
			...
			if use_batch then
				...
				for i = 1,num_chunks do 
					...
					x[i][2] = lookup:forward(train_utterances:select(1,shuffle[i])):clone()
